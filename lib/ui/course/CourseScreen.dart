...
import 'package:sanjivprkashan/ui/course/MyCourseListScreen.dart';
import 'package:sanjivprkashan/utils/styleUtil.dart';
import 'package:get/get.dart';

import '../../utils/AppBars.dart';
import 'CourseMainCatItemListScreen.dart';
import 'SubCatCourseScreen.dart';
...
class _CourseScreenState extends State<CourseScreen>
  ...
  Future<void> getData() async {
    await courseCtrl.getCourseCategory();
    if (_tabController != null) {
      _tabController?.dispose();
    }
    _tabController = TabController(
      length: courseCtrl.allTsCatList.length+0,
      vsync: this,
    );
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Obx(() {
          if (courseCtrl.isLoading.value || _tabController == null) {
            return Center(child: apiLoader());
          }

          if (courseCtrl.allTsCatList.isEmpty) {
            return Center(child: emptyWidget(context, "No study material found", onRefresh: getData));
          }
          ...
          return RefreshIndicator(
            ...
            child: Column(
              children: [
                otherAppBar(context, "Study Material", false),
                Flexible(child: _buildTabView(courseCtrl.allTsCatList)),
              ],
            ),
          );
        }),
      ...
      children: [
        Container(
          ...
          child: TabBar(
            ...
            isScrollable: (menuData.length + 0) <= 2 ? false : true,
  ...
  Widget _buildCategoryView(TSMainCatModel category, bool isDarkTheme, double screenWidth) {
    ...
    if (subItems.isEmpty) {
      // courseCtrl.isTestCourse.value = false;
      debugPrint("Showing AllTestSeriesCourseScreen for ${category.courseCategoryName}");
      return CourseMainCatItemListScreen(catID: category.courseCategoryId.toString(),title: category.courseCategoryName, isShowHeader: false,);
  ...
  Widget _buildGridItem(ChildCategory item, bool isDarkTheme, double screenWidth) {
    ...
    bool isExpanded = courseCtrl.expandedSubjectIds.contains(item.courseCategoryId);
    return Column(
      ...
      children: [
        InkWell(
          onTap: () {
            ...
            if (item.subchildren.isNotEmpty) {
              ...
              if (isExpanded) {
                courseCtrl.expandedSubjectIds.remove(item.courseCategoryId);
              } else {
                courseCtrl.expandedSubjectIds.add(item.courseCategoryId);
              }
              setState(() {
              });
            }else{
              Get.to(()=> SubCatCourseScreen(catID: item.courseCategoryId.toString(),title: item.courseCategoryName, isShowHeader: true,));
            }
          },
          child: Container(
        ...
        if (isExpanded)
          ...List.generate(item.subchildren.length ?? 0, (index) {
            ...
            return InkWell(
              onTap: (){
                Get.to(()=> SubCatCourseScreen(catID: myItem.courseCategoryId.toString(),title: myItem.courseCategoryName, isShowHeader: true,));
              },
              child: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Image.asset("assets/images/book.png", width: 24, height: 24),
                          SizedBox(width: 10,),
