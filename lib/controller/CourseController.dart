...
import 'package:sanjivprkashan/model/course/NewMyCourseModel.dart';
...
class CourseController extends GetxController{
  ...
  Future<void> getCourseCategory() async {
    ...
    try {
      var response = await http.get(Uri.parse(Constant.coursesCategories),
        // body: userData,
  ...
  RxList<NewMyCourseModel> myeCourseList = RxList<NewMyCourseModel>();
  Future<void> getMyCourseList() async {
    isLoading.value = true;
    myeCourseList.clear();
    try {
      var response = await http.get(Uri.parse(Constant.myCourses),
        // body: userData,
        headers: Constant().userHeader,
      );
      if (response.statusCode == 200) {
        ...
        if (data['status'] == "success") {
          ...
          myeCourseList.addAll((data["allCourses"] as List)
              .map((e) => NewMyCourseModel.fromJson(e))
              .toList());
        }else{
          if(data["message"] == "Session Expired"){
            SessionManager().logout();
